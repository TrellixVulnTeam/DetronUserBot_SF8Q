...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    ULTRONevent = await edit_or_reply(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await ULTRONevent.edit("`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    if event.fwd_from:
        return
    ULTRONevent = await edit_or_reply(event, "...")
    start = datetime.now()
    ...
    try:
        ...
        await ULTRONevent.edit(str(e))
        return
    end = datetime.now()
    ms = (end - start).seconds
    await ULTRONevent.edit("Created BarCode in {} seconds🤓".format(ms))
...
async def _(event):
    if event.fwd_from:
        return
    ULTRONevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await ULTRONevent.edit(output_str, link_preview=True)
    else:
        await ULTRONevent.edit("xkcd n.{} not found!".format(xkcd_id))
