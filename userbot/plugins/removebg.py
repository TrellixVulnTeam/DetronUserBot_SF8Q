...
async def remove_background(event):
    ...
    remove_bg_image = "DETRONBOT.png"
    if "image" in contentType:
        with open("DETRONBOT.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await edit_or_reply(LEGENDevent, f"`{response.content.decode('UTF-8')}`")
        return
    if cmd == "srmbg":
        file = convert_tosticker(remove_bg_image, filename="DETRONBOT.webp")
